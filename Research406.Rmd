...
shootings$Shooter.Age[which(naremoved$Shooter.Age == 40888)] <- 12
...
agg.suicide <- suicide.by.year %>% mutate(decade = floor(Year/10)*10) %>% 
      group_by(decade) %>% 
      summarize_all(sum) %>% 
      select(-Year)
agg.suicide$percentage <- agg.suicide$suicide/agg.suicide$count
agg.suicide
summary(lm(percentage ~ decade, agg.suicide))
```
